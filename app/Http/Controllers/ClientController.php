...
class ClientController extends Controller
{
   ...
   public function get_new_leads(Request $request)
    ...
    public function assign_leads_to_company(Request $request)
    {
        try {
            ...
            if ($j == $i) {
                return json_encode(array('status' => 'success', 'msg' => 'Company assigned successfully!!'));
            } else {
    ...
    public function multi_convert_client(Request $request)
    {
        try {
            ...
            if ($j == $i) {
                return json_encode(array('status' => 'success', 'msg' => 'Client converted successfully!!'));
            } else {
    ...
    public function lead_complete(Request $request)
    {
        try {
            ...
            $j = 0;
            for ($i = 0; $i < sizeof($lead_id); $i++) {
                $check_complete = DB::table('leads')->where('id', $lead_id[$i])->where('complete', 'yes')->count();
                if ($check_complete) {
                    $client_name[$i] = DB::table('leads')->where('id', $lead_id[$i])->where('complete', 'yes')->value('name');
                    if (!in_array($client_name[$i], $completeClient)) {
                        array_push($completeClient, $client_name[$i]);
                    }
                }
            }
            if (!empty($completeClient) && sizeof($completeClient) > 0) {
                return json_encode(array('status' => 'fail', 'msg' => 'Mark already exist for  leads ' . implode(',', $completeClient) . '  !!'));
            }
            for ($i = 0; $i < sizeof($lead_id); $i++) {
            ...
            if ($j == $i) {
                return json_encode(array('status' => 'success', 'msg' => 'Mark complete successfully !!'));
            } else {
                return json_encode(array('status' => 'fail', 'msg' => 'Something went wrong while mark completion !!'));
            }
        } catch (QueryException $e) {
            Log::error("Database error ! [" . $e->getMessage() . "]");
            return redirect()->back()->with('alert-danger', 'something went wrong. please try again');
    ...
    public function lead_delete(Request $request)
    {
        try {
            ...
            if ($j == $i) {
                return json_encode(array('status' => 'success', 'msg' => 'Mark deleted successfully !!'));
            } else {
                return json_encode(array('status' => 'fail', 'msg' => 'Something went wrong while mark deletion !!'));
            }
        } catch (QueryException $e) {
            Log::error("Database error ! [" . $e->getMessage() . "]");
            return redirect()->back()->with('alert-danger', 'something went wrong. please try again');
    ...
    public function leads_timeline(Request $request)
    {
      ...
      public function campaign_wise_leads(){
        $campaigns = DB::table('leads')
        ...
        return view('pages.clients.campaign_wise_leads_report',['campaigns' => $campaigns]);
    }
    ...
    public function get_lead_report(Request $request)
    {
        try {
            ...
            ->select(
                ...
                'leads.society_name',
                'leads.email',
                'leads.mobile_no as contact',
                'leads.address',
                'leads.lead_source as source_name',
                DB::raw('COALESCE(lead_type.type, "Unknown") as type'),
                'leads.campaign_name',
                DB::raw('COUNT(lead_follow_up.id) as total_follow_ups'),
                DB::raw('0 as total_appointments'),
                DB::raw('0 as total_quotations'),
                DB::raw('"new leads" as client_leads')
            )
            ->leftJoin('lead_type','lead_type.id','leads.lead_type')
            ->leftJoin('quotation', 'leads.id', '=', 'quotation.client_id')
            ->leftJoin('lead_follow_up', 'leads.id', '=', 'lead_follow_up.client_id')
            ->where('leads.campaign_id',$selectedCampaignId)
            ->groupBy(
                'leads.name',
                'leads.email',
                'leads.mobile_no'
        ...
        $clients = DB::table('clients')
            ->select(
                'client_contacts.name as client_name',
                DB::raw('" " as society_name'),
                'client_contacts.email',
                'client_contacts.contact',
                'clients.address',
                'source.source as source_name',
                DB::raw('COALESCE(lead_type.type, "Unknown") as type'),
                'clients.campaign_name',
                DB::raw('COUNT(follow_up.id) as total_follow_ups'),
                DB::raw('COUNT(appointment.id) as total_appointments'),
                DB::raw('COUNT(quotation.id) as total_quotations'),
                'clients.client_leads'
            )
            ->leftJoin('client_contacts', 'clients.id', '=', 'client_contacts.client_id')
            ->leftJoin('follow_up', 'clients.id', '=', 'follow_up.client_id')
            ->leftJoin('appointment', 'clients.id', '=', 'appointment.client')
            ->leftJoin('quotation', 'clients.id', '=', 'quotation.client_id')
            ->leftJoin('lead_type','lead_type.id','clients.lead_type')
            ->leftJoin('source', 'source.id', '=', 'clients.source')
            ->where('clients.campaign_id', $selectedCampaignId)
            // ->whereNotNull('client_contacts.name')
            ->groupBy(
                'client_contacts.name',
                'client_contacts.email',
                'client_contacts.contact'
            );
            $results = $leads->union($clients)->get();
            if ($results != '') {
  ...
  public function campaign_wise_total(){
        $leads = DB::table('leads')
        ->leftJoin('lead_type', 'leads.lead_type', '=', 'lead_type.id')
        ->select(
            'campaign_id',
            'campaign_name',
            'lead_type.type as lead_type',
            DB::raw('COUNT(DISTINCT leads.id) as total_count'),
            DB::raw('0 as converted_clients'),
            DB::raw('0 as total_quotations'),
            DB::raw('DATE(leads.created_at) as created_date')
        )
        // ->groupBy('leads.campaign_id', 'lead_type.type')
        ->groupBy('leads.campaign_id', 'lead_type.type', DB::raw('DATE(leads.created_at)'))
        ->whereNotNull('campaign_id')
        ->orderBy(DB::raw('DATE(leads.created_at)'), 'desc')
        ->get();

        $clients = DB::table('clients as c')
        ->leftJoin(DB::raw('(SELECT client_id, COUNT(*) as quotation_count FROM quotation GROUP BY client_id) as q'), 'c.id', '=', 'q.client_id')
        ->leftJoin('lead_type', 'c.lead_type', '=', 'lead_type.id')
        ->select(
            'c.campaign_id',
            'c.campaign_name',
            'lead_type.type as lead_type',
            DB::raw('COUNT(DISTINCT c.id) as total_count'),
            DB::raw('COUNT(c.id) as converted_clients'),
            DB::raw('COALESCE(SUM(q.quotation_count), 0) as total_quotations'),
            DB::raw('DATE(c.created_at) as created_date')
        )
        // ->groupBy('c.campaign_id', 'lead_type.type')
        ->groupBy('c.campaign_id', 'lead_type.type', DB::raw('DATE(c.created_at)'))
        ->whereNotNull('campaign_id')
        ->orderBy(DB::raw('DATE(c.created_at)'), 'desc')
        ->get();

        $campaigns = $leads->merge($clients)->groupBy('campaign_id');
        $campaignsArray = $campaigns->toArray();

        // Sort each campaign group by created_date in descending order
        foreach ($campaignsArray as &$group) {
            usort($group, function($a, $b) {
                return strtotime($b->created_date) <=> strtotime($a->created_date);
            });
        }
        $lead_type = DB::table('lead_type')->get('type');
        return view('pages.clients.campaign_wise_total_report',['campaigns' => $campaignsArray,'lead_type'=> $lead_type]);
    }
    
     public function save_new_leads(Request $request)
    {
        try {
            $v = Validator::make($request->all(), [
            ...
            if($insert)
            {
                return response()->json(array('status' => 'success', 'msg' =>'Leads save successfully'));
            }
            else
            {
                return response()->json(array('status' => 'error', 'msg' =>'Leads can`t be saved'));
            }
     ...
     public function leads_followup_report(Request $request)
    {
        try {
          
            
             return view('pages.leads_followup_report');
        } catch (QueryException $e) {
            Log::error("Database error ! [" . $e->getMessage() . "]");
            return response()->json(array('error' => 'Database error'));
        }
    }
        public function filter_leads_followup(Request $request)
    {
        try {
            $from_date =$request->from_date;
            $to_date= $request->to_date;
            $data=DB::table('clients')
            ->join('follow_up','follow_up.client_id','clients.id')
            ->select('clients.id','clients.case_no','clients.client_name','clients.case_no',DB::raw('COUNT(follow_up.client_id) as followup_count'))
            ->groupBy('follow_up.client_id')
            ->having('followup_count','<',5)
            ->where('clients.client_leads','leads')
            ->where('clients.lead_type','!=',6)
            ->get();
            foreach($data as $row)
            {
                 $contact=DB::table('client_contacts')->select(DB::raw('GROUP_CONCAT(contact SEPARATOR ", ") as mobile'))->where('client_id',$row->id)->groupBy('client_id')->get();
                 if(sizeof($contact)==0)
                 {
                     $val=["mobile"=>""];
                     $row->contact=json_encode([$val]);
                 }
                 else{
                    $row->contact=$contact;
                 }
                 $email_id=DB::table('client_contacts')->select(DB::raw('GROUP_CONCAT(email SEPARATOR ", ") as email'))->where('client_id',$row->id)->groupBy('client_id')->get();
                 if(sizeof($email_id)==0)
                 {
                     $val1=["email"=>""];
                     $row->email=json_encode([$val1]);
                 }
                 else{
                    $row->email=$email_id;
                 }
            ...
            return view('pages.get_leads_followup_report',compact('data'));
        } catch (QueryException $e) {
            Log::error("Database error ! [" . $e->getMessage() . "]");
            return response()->json(array('error' => 'Database error'));
        }
