    ...
    success: function (data) {
      ...
      $("#donut-chart-TaskStatus .apexcharts-pie-series").on("click", function () {
        ...
        console.log(task_status_id);
         window.open("task_status_details-" + task_status_id+'-'+month ,"_self");
      });
    },
    error: function (data) {
      // Handle error
    ...
    var barChartconfig = {
      ...
      $('#bar_chart').click(function(event) {
        ...
        if (activePoints.length > 0) {
          ...
          var month=Type_month;
          console.log("Clicked bar:", typeName);
          window.open("task_type_details-" + typeName+'-'+month,"_self");
        }
      });

function donutChartClickDetail(task_status_id,month) {
  $.ajaxSetup({
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    }
...
function barChartClickDetail(task_type,month) {
  $.ajaxSetup({
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    }
