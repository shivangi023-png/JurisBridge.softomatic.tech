    ...
    public function handle()
    {
        try {
            ...
            foreach ($staff as $row) {
                ...
                $row->appointmentSchedule=$appointmentSchedule;
                ...
                $curdate= date('Y-m-d');
               $row->newTask = DB::table('task')->leftjoin("projects", "projects.id", "task.project_id")->leftjoin("task_status_master", "task_status_master.id", "task.status")->select('task.id', 'task.title', 'projects.project_name', "task_status_master.status as task_status", "task.start_date", "task.end_date")
               ->whereJsonContains('task.assignee', $staff_id)
                ->whereDate('task.start_date','<=',$curdate)
                ->whereDate('task.end_date','>=',$curdate)
                ->get();
                
                 $overdue_task_list=DB::table("task")
            ...
            $overdue_task_list = $overdue_task_list->where('task.end_date','<',$curdate)->whereNotIn('task.status',$status_arr)->orderBy('end_date','desc');
            $overdue_task_list = $overdue_task_list->whereJsonContains('task.assignee',$staff_id)->get($fields);
                $newTask = $row->newTask;
                ...
                $reply_to = null;
                $reply_to_name = null;
                $mailsent = PhpMailerController::sendEmail($to_email, null, null, view('pages.emails.send_mail_to_sales', compact('company', 'appointmentSchedule', 'appointmentMeet', 'newTask','overdue_task_list')), $subject, null, $mailer_username, $mailer_password, $mailer_name, $email_from, $reply_to, $reply_to_name);
            }
        } catch (\Exception $e) {
