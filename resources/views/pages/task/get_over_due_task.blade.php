          ...
          <div class="task_div" >
            ...
            <div class="col-sm-12 todoCBox project_task" class="" data-project_id="{{$row2->project_id}}" data-task_id="{{$row2->id}}" data-time="{{$row2->time}}" data-task_title="{{$row2->title}}" data-file_link="{{$row2->file_link}}"  data-task_type="{{$row2->type}}" data-task_priority="{{$row2->priority}}" data-task_assignee="{{$row2->assignee}}" data-task_status="{{$row2->status}}" data-task_is_milestone="{{$row2->is_milestone}}" data-task_start_date="{{($row2->start_date != null) ? date('d/m/Y',strtotime($row2->start_date)) : '' }}" data-task_end_date="{{($row2->end_date != null) ? date('d/m/Y',strtotime($row2->end_date)) : '' }}" data-task_description="{{$row2->description}}" data-office_id="{{$row2->office_id}}" data-working_hr="{{$row2->working_hr}}">
                        ...
                        <div class="TaskBox" style="margin-bottom:10px">
                           ...
                           <div class="commentBox row">
                               <i class="bx bx-calendar-x"></i>&nbsp;<h6>{{$row2->overdue_days}}</h6>
                           </div>
