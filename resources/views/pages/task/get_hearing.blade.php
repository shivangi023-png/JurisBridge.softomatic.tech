...
<div class="TaskHearingBox" style="padding-left:14px;padding-right:40px">
    <div>
        <h6 style="color:#5a8dee">Total Hearing : {{sizeof($task_list)}}<a href="{{url('/')}}/task_hearing_excel" class="ml-1 excelBtn" id="task_hearing_excel"  target="_blank">Download <img src="{{asset('images/icon/excelIcon.png')}}" alt="Excel" height="24" width="24"></a></h6>
    </div><br>
    @foreach($task_list as $row)
    <div class="row project_task" data-project_id="{{$row->project_id}}" data-task_id="{{$row->id}}" data-time="{{$row->time}}" data-task_title="{{$row->title}}" data-file_link="{{$row->file_link}}" data-task_type="{{$row->type}}" data-task_priority="{{$row->priority}}" data-task_assignee="{{$row->assignee}}" data-task_status="{{$row->status}}" data-task_is_milestone="{{$row->is_milestone}}" data-task_start_date="{{($row->start_date != null) ? date('d/m/Y',strtotime($row->start_date)) : '' }}" data-task_end_date="{{($row->end_date != null) ? date('d/m/Y',strtotime($row->end_date)) : '' }}" data-task_description="{{$row->description}}" data-office_id="{{$row->office_id}}" data-working_hr="{{$row->working_hr}}">
