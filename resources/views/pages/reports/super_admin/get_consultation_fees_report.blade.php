...
<body>
    ...
    <?php $i = 1; ?>
    @if (!empty($company) && ($comp->total_fees)>0)
    <h4 style="text-align:left;">Company - {{$comp->company_name}}</h4>
    <h5 style="text-align:right;">Total Consultation Fees- {{AppHelper::moneyFormatIndia($comp->total_fees)}}</strong></h5>
    @if (sizeof($comp->clients))
