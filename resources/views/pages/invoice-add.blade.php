                ...
                <div class="invoice-date-picker d-flex align-items-center justify-content-xl-end flex-wrap">
                  <div class="d-flex align-items-center">
                    ...
                    <fieldset class="d-flex ">
                      <input type="text" class="form-control pickadate mr-2 bill_date" placeholder="Select Date">
                    </fieldset>
            ...
            <div class="row mb-1">
              <div class="col-sm-2 col-6">
                <div class="radio d-inline-block">
                        <input type="radio" name="invoice_type" value="tax" class="invoice_type" id="radio">
                        <label for="radio">Tax Invoice</label>
                <span class="invoice_type_err valid_err"></span>
                </div>
              </div>
              <div class="col-sm-3 col-6">
                <div class="radio d-inline-block">
                        <input type="radio" name="invoice_type" value="proforma" class="invoice_type" id="radio1">
                        <label for="radio1">Proforma Invoice</label>
                    </div>
              </div>
            </div>
                ...
                <fieldset class="form-group">
                  <div class="input-group">
                    ...
                    <select class="form-control sign" name="sign">
                      ...
                      @if($staff->sid==1 || $staff->sid==10 || $staff->sid==15 || $staff->sid==7 || $staff->sid==9 || $staff->sid==2 || $staff->sid==12 || $staff->sid==6)
                      @if($staff->sid==1)
    ...
    <div class="card invoice-action-wrapper shadow-none border">
      <div class="card-body">
        <div class="invoice-action-btn mb-1">
          <button class="btn btn-primary btn-block invoice-send-btn preview">
...
<script>
 ...
 $(document).on('click', '.preview', function(e) {
  ...
  e.preventDefault();
      ...
      if (valid) {
        var preview_url = "{{url('invoice_preview')}}" + "?client="+client+"&bill_date="+bill_date+"&due_date="+due_date+"&seal="+seal+"&sign="+sign+"&service[]="+service_array+"&amount[]="+amount_array+"&quotation="+quo_array+"&bank="+bank+"&company="+company+"&gst_percent[]="+GST_percent_array+"&gst_amount[]="+GST_amount_array+"&cgst_percent[]="+CGST_percent_array+"&cgst_amount[]="+CGST_amount_array+"&igst_percent[]="+IGST_percent_array+"&igst_amount[]="+IGST_amount_array+"&discount[]="+discount_array+"&total_amount[]="+total_amount_array+"&round_check[]="+round_check_array+"&invoice_type="+invoice_type+"&description="+description;
    ...
    function validateinvoice() {
      ...
      if ($('.invoice_type:checked').val() == '' || $('.invoice_type:checked').val() == undefined) {
       $('.invoice_type_err').html('Invoice required');
       valid = false;
      }
      if ($('.client').val() == "") {
